...
## M6 outcome (landed on `main`, 2026-09-19)

Packages `gophermind-lib/lockfile`, `plantree/plan`, `plantree/export` (new) and `tui`, fourteen commits `e7e67a6..a3f5481`. The whole workflow is now wired: `/project <name> <brief-file>` reads the brief, runs pass 1 (chunked, fresh context, progress "chunk i of n") and pass 2 under one run lock with prompt sizes derived from the model's context window (`plan.SizesFor`, with a warning when `FitsWindow` is false), shows one question round, then an approval prompt; approving runs `plan.Approve` then `export.ExportLegacy`, which writes `ROADMAP.md`, `assignments.json` (one row per task, agent `executor`, model `strong`, no dependencies so the executor runs them one at a time), `SPEC.md`, `PROJECT.md` and the approval marker, gated by `phaseflow.ValidatePlan`. `/questions` resumes the round and hands over to the same approval prompt. The interview and `generationPrompt` code is gone. A missing brief file is an error. The owner's original failure (a brief overflowing a 98,304-token window) is addressed at every step: no pass sees more than a bounded chunk, overview or batch.

### Carry-forward (after M6; nothing here blocks use)

1. **Waves and parallelism.** No task dependencies or waves are exported; add task-level dependencies (a pass-1 field or a rule) and `is_contract`, agent/model per task.
2. **Node removal** (a skipped step still blocks approval), and changing an answer cannot delete steps a new answer makes unnecessary.
3. **Inline brief text** is not supported (a file path only). `gophermind-osx/ui/breakdown.go` still seeds the old agent-writes-the-files interview against the same `.planning/` files: the desktop and TUI paths disagree about who owns them.
4. **"Already exported" is the marker alone** (`tui/approve.go`): if `ROADMAP.md` or `assignments.json` are deleted or edited after a good export, `/project` still says exported and offers no re-export. Stat the files or offer "export again".
5. `ChangeAnswer`'s `ErrRunBusy` reaches the owner raw; the window probe before the first transcript line has no deadline of its own (up to 60 s silent gap); `|` in a task title breaks PROJECT.md's table (`phaseflow/projectdoc.go`); `/project-execute` already past its approval gate is not blocked during a re-export.
6. The M5 low items still stand (see "M5 outcome"), plus: a same-answer `ChangeAnswer` after a re-plan re-flags the steps; the `BRIEF EXCERPTS` terminator; `INDEX.md` (generated by `gophermind index`) predates plantree; `approveAndExport` runs synchronously on the UI goroutine (about 0.2 s, no model call).
7. Byte-per-token ratio of 3 is a mostly-ASCII assumption: a CJK-heavy brief on a small window can still overflow; below about 8,390 tokens nothing is guaranteed to fit.
