...
func (a *Agent) Send(ctx context.Context, userInput string) (string, error) {
	...
	// Tool failure tracking: if a tool fails 3+ times, stop retrying it
	...
	for i := 0; i < a.maxIter; i++ {
		...
		for _, call := range reply.ToolCalls {
			...
			// Track tool failures: if a tool fails 3+ times, abort to avoid wasting iterations
			if strings.HasPrefix(out, "error:") {
				toolFailures[call.Function.Name]++
				if toolFailures[call.Function.Name] >= maxToolFailures {
					a.msgs = a.msgs[:base]
					return "", fmt.Errorf("aborted: %s failed %d times in a row (last error: %s): %w",
						call.Function.Name, maxToolFailures, out, ErrMaxIterations)
				}
			} else {
